            ...
            <table class="table-auto w-full text-left border-collapse border border-[#E5E7E9]">
              ...
              <tbody>
                <tr v-for="(item, index) in filteredAndPaginatedData" :key="`${index}-${item.judul}`" class="bg-[#FFFFFF] border border-[#E5E7E9] text-[12px] text-[#4D5E80] font-sans font-semibold">
                  ...
                  <td v-if="item.date > 0" class="p-2 py-4 border border-[#E5E7E9] text-[#18c429]">H-{{ item.date }}</td>
                  <td v-else-if="item.date == 0" class="p-2 py-4 border border-[#E5E7E9] text-[#f9c01a]">H</td>
                  <td v-else-if="item.date < 0" class="p-2 py-4 border border-[#E5E7E9] text-[#FF5656]">H+{{ item.date * -1 }}</td>
                  ...
                  <td class="p-2 py-4 border border-[#E5E7E9] relative">
                    ...
                    <div v-if="actionDropdownIndex === index" class="absolute right-[45px] bottom-[10px] h-[40px] flex items-center rounded-lg bg-[#FFFFFF] border border-[#E5E7E9] hover:bg-[#DBEAFE] shadow-lg z-10">
...
import { mapperStatus, dueDateParsing } from '@/utils/helper';
  ...
  methods: {
    ...
    navigateToDetail(tipe, id) {
      if (tipe.toUpperCase() === "PKS") {
        this.$router.push(`/proses/detailpengajuanpks/${id}`);
      } else if (tipe.toUpperCase() === "MOU" || tipe.toUpperCase() === "NDA") {
        this.$router.push(`/proses/detailpengajuanmou/${id}`);
      }
    },
    toggleDropdown() {
    ...
    // api
      ...
      if (positionLevel == "PartnershipManager") {
        url = "mitra/manager/mounda/proses";
      }
      if (positionLevel == "PartnershipVP") {
        url = "mitra/vp/mounda/proses";
      }
      ...
      if (!url) {
        ...
        return this.modalFailed = {
          ...
          bisnis_type: item.bisnisType,
					user: item.user,
          date: item.isStopClock? "-": dueDateParsing(item.dueDateStaff),
      ...
      let url2 = null;
      if (positionLevel == "PartnershipManager") {
        url2 = "mitra/manager/pks/proses";
      }
      if (positionLevel == "PartnershipVP") {
        url2 = "mitra/vp/pks/proses";
      }
      if (positionLevel == "PartnershipDirector") {
      ...
      this.filterOptions[1].subOptions = [...new Set(boxResult.map(item => item.progress))];
      this.isLoading = false;
		}
  },
  mounted() {
