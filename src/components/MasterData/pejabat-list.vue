<template>
  <div v-if="showFormPopup" class="fixed inset-0 flex items-center justify-center bg-[#1F2937] bg-opacity-50">
    <div class="w-[684.92px] rounded-lg shadow-lg bg-[#FFFFFF] border-collapse pb-12">
      ...
      <h1 class="font-sans text-[30px] text-center text-[#333333] mt-6 ml-[5px] font-semibold">Form Pejabat</h1>
      <div class="w-full h-[400px] overflow-y-auto px-10 pb-5">
        <div v-if="[1, 2].includes(formType)" class="mb-4">
          <label class="text-[#4D5E80] font-medium">Nama Lengkap <span class="text-[#FF5656]">*</span></label>
          <form action="" class="w-full py-[10px] px-4 mt-2 border-[1px] rounded-lg text-sm flex justify-between">
            <input v-model="fullName" type="text" placeholder="Nama Lengkap..." class="w-full outline-none">
          </form>
        </div>
        <div v-if="[1, 2].includes(formType)" class="mb-4">
          <label class="text-[#4D5E80] font-medium">Jabatan <span class="text-[#FF5656]">*</span></label>
          <form action="" class="w-full py-[10px] px-4 mt-2 border-[1px] rounded-lg text-sm flex justify-between">
            <input v-model="title" type="text" placeholder="Jabatan..." class="w-full outline-none">
          </form>
        </div>
        <div class="flex justify-center mt-8">
          ...
          }" :disabled="!isSendAvaible" @click="SendCreatePejabat"
          ...
          }" :disabled="!isSendEditAvaible" @click="SendEditPejabat"
            class="p-3 flex justify-center w-full rounded-lg border-[1px] text-[16px] text-center font-sans font-semibold">
            Edit Pejabat
          </button>
        </div>
  ...
  <div class="px-4 py-3">
    <div class="h-auto w-[1086px] rounded-lg bg-[#FFFFFF] border-collapse mx-auto">
      <div class="flex">
        <h1 class="mt-[20px] ms-8 text-[30px] font-sans text-[#4D5E80] font-bold">Master Pejabat</h1>
      </div>
      <div>
      ...
      <div class="flex">
        <div class="flex w-[1046px] rounded-lg bg-[#FFFFFF] border-[1px] border-[#E5E7E9] mt-4 ml-4 mr-4 overflow-auto">
          <table class="table-auto w-full text-left border-collapse border border-[#E5E7E9]">
            ...
            <tbody>
              <tr v-for="(item, index) in filteredAndPaginatedData" :key="`${index}-${item.fullName}`"
                ...
                <td class="p-2 py-4 border border-[#E5E7E9]">{{ (currentPage - 1) * selectedValue +
                  index + 1 }}</td>
                <td class="p-2 py-4 border border-[#E5E7E9]">{{ item.fullName }}</td>
                <td class="p-2 py-4 border border-[#E5E7E9]">{{ item.title }}</td>
                <td class="p-2 py-4 border border-[#E5E7E9] relative">
                  ...
                  <div v-if="actionDropdownIndex === index"
                    ...
                    <button @click="SendDeletePejabat(item.id)"
...
export default {
  ...
  computed: {
    ...
    isSendEditAvaible() {
      return (
        this.fullName != "" && this.title != "" && this.pejabatId
      )
    },
  },
  methods: {
    ...
    openCreatePejabat() {
      ...
      this.postCreatePejabat(this.successCreatePejabat, this.failCreatePejabat);
    },
    closeCreatePejabat() {
      this.closeModalDialog()
    },
    ...
    openEditPejabat() {
      this.closeModalDialog();
      this.postEditPejabat(this.successEditPejabat, this.failEditPejabat);
    },
    closeEditPejabat() {
      this.closeModalDialog()
    },
    ...
    // Popup Delete Pejabat
    SendDeletePejabat(id) {
      this.pejabatId = id
      this.modalDialog = {
        isVisible: true,
        title: 'Hapus Pejabat',
    ...
    openDeletePejabat() {
      this.closeModalDialog();
      this.postDeletePejabat(this.successDeletePejabat, this.failDeletePejabat);
    },
    closeDeletePejabat() {
      this.closeModalDialog()
    },
    ...
    async postCreatePejabat(successFunction, failFunction) {
      this.isLoading = true;
      ...
      const res = await fetchPostForm('functionary', null, form, this.$router);
      console.log(res.data)
      if (res.status == 201) {
        this.isLoading = false;
        successFunction();
    ...
    async postEditPejabat(successFunction, failFunction) {
      this.isLoading = true;
      const form = new FormData()
      form.append('fullName', this.fullName);
      form.append('title', this.title);
      ...
      const res = await fetchPutForm(`functionary/${this.pejabatId}`, null, form, this.$router);
      console.log(res.data)
      if (res.status == 201) {
        this.isLoading = false;
        successFunction();
    ...
    async postDeletePejabat(successFunction, failFunction) {
      this.isLoading = true;
      const res = await fetchDelete(`functionary/${this.pejabatId}`, null, this.$router);
      console.log(res.data)
      if (res.status == 200) {
        this.isLoading = false;
        successFunction();
