<template>
  <div v-if="showFormPopup" class="fixed inset-0 flex items-center justify-center bg-[#1F2937] bg-opacity-50">
    <div class="w-[684.92px] rounded-lg shadow-lg bg-[#FFFFFF] border-collapse pb-12">
      ...
      <h1 class="font-sans text-[30px] text-center text-[#333333] mt-6 ml-[5px] font-semibold">Form Produk</h1>
      <div class="w-full h-[400px] overflow-y-auto px-10 pb-5">
        <div v-if="[1, 2].includes(formType)" class="mb-4">
          <label class="text-[#4D5E80] font-medium">Nama Produk<span class="text-[#FF5656]">*</span></label>
        ...
        <div class="flex justify-center mt-8">
          ...
          }" :disabled="!isSendAvaible" @click="SendCreateProduk"
          ...
          }" :disabled="!isSendEditAvaible" @click="SendEditProduk"
            class="p-3 flex justify-center w-full rounded-lg border-[1px] text-[16px] text-center font-sans font-semibold">
            Edit Produk
          </button>
        </div>
  ...
  <div class="px-4 py-3">
    <div class="h-auto w-[1086px] rounded-lg bg-[#FFFFFF] border-collapse mx-auto">
      <div class="flex">
        <h1 class="mt-[20px] ms-8 text-[30px] font-sans text-[#4D5E80] font-bold">Master Produk</h1>
      </div>
      <div>
              ...
              <tr v-for="(item, index) in filteredAndPaginatedData" :key="`${index}-${item.name}`"
                ...
                <td class="p-2 py-4 border border-[#E5E7E9] relative">
                  ...
                  <div v-if="actionDropdownIndex === index"
                    ...
                    <button @click="SendDeleteProduk(item.id)"
  ...
  methods: {
    ...
    SendCreateProduk() {
      this.modalDialog = {
        ...
        okFunction: this.openCreateProduk,
        closeFunction: this.closeCreateProduk
      }
    },
    openCreateProduk() {
      this.closeModalDialog();
      this.postCreateProduk(this.successCreateProduk, this.failCreateProduk);
    },
    closeCreateProduk() {
      this.closeModalDialog()
    },
    successCreateProduk() {
      this.modalSuccess = {
        ...
        closeFunction: this.closeSelesaiCreateProduk
      }
    },
    failCreateProduk(data) {
      this.modalFailed = {
        isVisible: true,
        title: 'Gagal',
    ...
    SendEditProduk() {
      this.modalDialog = {
        ...
        title: 'Edit Produk',
        message: 'Apakan anda yakin dengan data yang anda masukan',
        okFunction: this.openEditProduk,
        closeFunction: this.closeEditProduk
      }
    },
    openEditProduk() {
      this.closeModalDialog();
      this.postEditProduk(this.successEditProduk, this.failEditProduk);
    },
    closeEditProduk() {
      this.closeModalDialog()
    },
    successEditProduk() {
      this.modalSuccess = {
        ...
        closeFunction: this.closeSelesaiEditProduk
      }
    },
    failEditProduk(data) {
      this.modalFailed = {
    ...
    SendDeleteProduk(id) {
      ...
      this.modalDialog = {
        isVisible: true,
        title: 'Hapus Produk',
        message: 'Apakan anda yakin menghapus Produk ini',
        okFunction: this.openDeleteProduk,
        closeFunction: this.closeDeleteProduk
      }
    },
    openDeleteProduk() {
      this.closeModalDialog();
      this.postDeleteProduk(this.successDeleteProduk, this.failDeleteProduk);
    },
    closeDeleteProduk() {
      this.closeModalDialog()
    },
    successDeleteProduk() {
      this.modalSuccess = {
        ...
        closeFunction: this.closeSelesaiDeleteProduk
      }
    },
    failDeleteProduk(data) {
      this.modalFailed = {
        isVisible: true,
        title: 'Gagal',
        message: data?.message ? data.message : "Silahkan hubungi admin"
    ...
    async postCreateProduk(successFunction, failFunction) {
      this.isLoading = true;
    ...
    async postEditProduk(successFunction, failFunction) {
      this.isLoading = true;
      const form = new FormData()
      form.append('productName', this.name);
      // Display the values
